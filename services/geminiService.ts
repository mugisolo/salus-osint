...
export const analyzePresidentialCandidate = async (name: string, party: string): Promise<PresidentialProfile | null> => {
  ...
  const prompt = `
    You are "Mugi-Solo", a deep OSINT intelligence persona.
    Conduct a forensic analysis on the Ugandan presidential candidate: ${name} (${party}) for the 2026 election.
    ...
    **IMPORTANT: For the 'grandStrategy' field:**
    You are consulting "The Grand Strategist." Synthesize the political wisdom of historical figures (Sun Tzu, Machiavelli, etc.) to create a cohesive strategy.
...
export const generatePoliticalStrategy = async (
  ...
  const prompt = `
    You are "Mugi-Solo", an advanced AI engine synthesizing the political, military, and strategic wisdom of history's greatest minds.
...
export const generateDailyOpEd = async (
  ...
  const prompt = `
    Act as "Mugi-Solo", the Senior Political Analyst for "Salus Intelligence," producing the daily 04:30 AM briefing on the 2026 Ugandan Election.
...
export const generateDeepMindAnalysis = async (query: string): Promise<{
  ...
  const prompt = `
    You are "Mugi-Solo", an advanced strategic intelligence engine focused on Ugandan Politics.
  ...
  try {
    const chat = ai.chats.create({
      ...
      config: {
        tools: [{ googleSearch: {} }],
        thinkingConfig: { thinkingBudget: 32768 },
        systemInstruction: `You are Mugi-Solo, the Salus Intelligence Analyst AI.
